...
class Inputs {
    ...
    let clearSale: Signal<Void>
    init(nozzle1: Signal<UpDown>, nozzle2: Signal<UpDown>, nozzle3: Signal<UpDown>, keyPad: Signal<Key>, fuelPulses: Signal<Int>, calibration: BehaviorRelay<Double>, price1: BehaviorRelay<Double>, price2: BehaviorRelay<Double>, price3: BehaviorRelay<Double>, clearSale: Signal<Void>) {
...
class ClearSalePump: Pump {
    func create(inputs: Inputs, disposeBag: DisposeBag) -> Outputs {
        ...
        let notifyPointOfSale = NotifyPointOfSale(lc: LifeCycle(nozzle1: inputs.nozzle1,
                                                                ...
                                                                disposeBag: disposeBag),
                                                  sClearSale: inputs.clearSale,
                                                  fi: fill)
        notifyPointOfSale.sStart.emit(to: start)

        let delivery = notifyPointOfSale.fuelFlowing.map { fuel -> Delivery in
            switch fuel {
            case .some(.one):
                return .fast1
            case .some(.two):
        ...
        let priceLCD1 = ShowDollarsPump.priceLCD(fillActive: notifyPointOfSale.fillActive,
                                                 fillPrice: fill.price,
                                                 fuel: .one,
                                                 inputs: inputs,
                                                 disposeBag: disposeBag)
        let priceLCD2 = ShowDollarsPump.priceLCD(fillActive: notifyPointOfSale.fillActive,
                                                 fillPrice: fill.price,
                                                 fuel: .two,
                                                 inputs: inputs,
                                                 disposeBag: disposeBag)
        let priceLCD3 = ShowDollarsPump.priceLCD(fillActive: notifyPointOfSale.fillActive,
                                                 fillPrice: fill.price,
                                                 fuel: .three,
                                                 inputs: inputs,
                                                 disposeBag: disposeBag)
...
class NotifyPointOfSale {
    ...
    init(lc: LifeCycle, sClearSale: Signal<Void>, fi: Fill) {
...
class ShowDollarsPumpViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let clearSale: Signal<Void> = v.saleOKButton.rx.tap.asSignal(onErrorSignalWith: .empty())
        ...
        let showDollarsPump = ClearSalePump().create(inputs: inputs, disposeBag: disposeBag)
